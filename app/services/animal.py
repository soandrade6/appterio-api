...
from app.models.detail import Diet, LastObservations, ClinicalSigns, Vaccines
...
def get_animals_by_researcher(db: Session, id: uuid.UUID):
    try:
        ...
        research_studies = db.query(Research).filter(Research.researcher_id == researcher.id).all()
        if not research_studies:
            return []
        
        specimen_ids = [study.specimen_id for study in research_studies]
        animals = db.query(Animal).filter(Animal.id.in_(specimen_ids)).all()
